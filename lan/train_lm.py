...
from . import nmt_model_lm
# from . import gnmt_model
from . import inference_lm as inference
from . import model_helper_lm as model_helper
from . import misc_utils as utils
from . import nmt_utils
...
def run_internal_eval(
  ...
  dev_src_file, dev_tgt_file = Data.validation()
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
  ...
  result_summary = _format_results("dev", dev_ppl
                                   #dev_scores, hparams.metrics
                                   )
  # if avg_dev_scores:
  #   result_summary += ", " + _format_results("avg_dev", None, avg_dev_scores,
  #                                            hparams.metrics)
  if hparams.test_prefix:
    result_summary += ", " + _format_results("test", test_ppl
                                             #,test_scores,hparams.metrics
                                             )
    # if avg_test_scores:
...
def train(hparams, scope=None, target_session=""):
  ...
  if not hparams.attention:
    ...
    if hparams.attention_architecture == "standard":
      model_creator = nmt_model_lm.AttentionModel
    else:

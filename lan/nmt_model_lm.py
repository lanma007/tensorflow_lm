...
from . import model_helper_lm as model_helper
from . import iterator_utils_lm as iterator_utils
from . import misc_utils as utils
from . import helper_lm
from . import basic_decoder_lm
from . import attention_wrapper_lm
...
class BaseModel(object):
  ...
  def _build_encoder(self, hparams):
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      cell, encoder_initial_state = self._build_encoder_cell(
        ...
        # Helper
      helper = helper_lm.LanEncoderHelper(
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        # Helper
        helper = helper_lm.LanHelper(
          target_input, iterator.target_sequence_length,
            time_major=self.time_major)

        # Decoder
        my_decoder = basic_decoder_lm.BasicDecoder(
            cell,
            helper,
  ...
  def _compute_loss(self, logits):
    ...
    #
    # loss = tf.reduce_sum(
    #     cost * target_weights) / tf.to_float(self.batch_size)
    loss = tf.reduce_sum(tf.pow(tf.subtract(logits, target_output), 2))
    return loss
...
class AttentionModel(BaseModel):
  ...
  def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
    ...
    attention_mechanism = self.attention_mechanism_fn(
      attention_option, num_units, memory, self.mode)

    beta_mechanism = self.attention_mechanism_fn(
      'LanBeta', num_units, memory, self.mode)
    ...
    alignment_history = (self.mode == tf.contrib.learn.ModeKeys.INFER and
                         beam_width == 0)
    cell = attention_wrapper_lm.AttentionWrapperWithBeta(
      cell,
      attention_mechanism,
      beta_mechanism,
  ...
  def _build_encoder_cell(self, hparams, num_layers):
    ...
    alignment_history = (self.mode == tf.contrib.learn.ModeKeys.INFER and
                         beam_width == 0)
    cell = attention_wrapper_lm.AttentionWrapper(
      cell,
      attention_mechanism,
      attention_layer_size=num_units,
      alignment_history=alignment_history,
    ...
    # if hparams.pass_hidden_state:
    #   encoder_initial_state = cell.zero_state(batch_size, dtype).clone(
    #     cell_state=encoder_state)
    # else:
    encoder_initial_state = cell.zero_state(batch_size, dtype)
...
def create_attention_mechanism(attention_option, num_units, memory, mode):
  """Create attention mechanism based on the attention_option."""
  del mode  # unused

  if attention_option == 'Lan':
    attention_mechanism = attention_wrapper_lm.LanAttention(
      num_units, memory)
  elif attention_option == 'LanBeta':
    attention_mechanism = attention_wrapper_lm.LanBetaAttention(
        num_units, memory)

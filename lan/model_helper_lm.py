...
def create_train_model(
  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    Data=GD.Input_data(batch_size=hparams.batch_size,n_step_encoder=hparams.src_max_len,\
                       n_step_decoder=hparams.tgt_max_len)
    src_file, tgt_file = Data.training()
    src_dataset = tf.contrib.data.Dataset.from_tensor_slices(src_file)
    tgt_dataset = tf.contrib.data.Dataset.from_tensor_slices(tgt_file)
    skip_count_placeholder = tf.placeholder(shape=(), dtype=tf.int64)

    iterator = iterator_utils_lm.get_iterator(
        src_dataset,
        tgt_dataset,
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "eval"):
    # src_vocab_table, tgt_vocab_table = vocab_utils.create_vocab_tables(
    #     src_vocab_file, tgt_vocab_file, hparams.share_vocab)
    src_file_placeholder = tf.placeholder(shape=[None,hparams.src_max_len,hparams.src_vocab_size], dtype='float')
    tgt_file_placeholder = tf.placeholder(shape=[None,hparams.src_max_len,hparams.src_vocab_size], dtype='float')
    # Data=GD.Input_data(batch_size=hparams.batch_size,n_step_encoder=hparams.src_max_len, \
    #                    n_step_decoder=hparams.tgt_max_len)
    # src_file, tgt_file = Data.validation()
    src_dataset = tf.contrib.data.Dataset.from_tensor_slices(src_file_placeholder)
    tgt_dataset = tf.contrib.data.Dataset.from_tensor_slices(tgt_file_placeholder)
    iterator = iterator_utils_lm.get_iterator(
        src_dataset,
        tgt_dataset,
        # src_vocab_table,
...
def create_infer_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "infer"):
    ...
    src_placeholder = tf.placeholder(shape=[None], dtype='float')
    batch_size_placeholder = tf.placeholder(shape=[], dtype=tf.int64)

    Data=GD.Input_data(batch_size=hparams.batch_size,n_step_encoder=hparams.src_max_len, \
                       n_step_decoder=hparams.tgt_max_len)
    src_file,_= Data.validation()
    src_dataset = tf.contrib.data.Dataset.from_tensor_slices(src_file)
    iterator = iterator_utils_lm.get_infer_iterator(
        src_dataset,
        # src_vocab_table,
...
def create_rnn_cell(unit_type, num_units, num_layers, num_residual_layers,
  ...
  Args:
    ...
    mode: either mtf.contrib.learn.TRAIN/EVAL/INFER
